  ...
  return (
    <>
      ...
      <main className="min-h-[70vh] flex flex-col items-center justify-center bg-[#fbf5f8] py-16">
        <h1 className="text-3xl font-extrabold text-[#8e1dd1] mb-4">Escáner Corporal Virtual</h1>
        ...
        {!enviado ? (
          <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow p-8 space-y-6 max-w-xl w-full">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block font-bold mb-1">Altura (cm) *</label>
                <input type="number" value={altura} onChange={e => setAltura(e.target.value)} className="w-full px-4 py-2 border rounded-lg" required min={100} max={250} />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
              ...
              <div>
                <label className="block font-bold mb-1">Cintura (cm) *</label>
                <input type="number" value={cintura} onChange={e => setCintura(e.target.value)} className="w-full px-4 py-2 border rounded-lg" required min={40} max={200} />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block font-bold mb-1">Cuello (cm) *</label>
                <input type="number" value={cuello} onChange={e => setCuello(e.target.value)} className="w-full px-4 py-2 border rounded-lg" required min={20} max={60} />
              </div>
              {sexo === "femenino" && (
                <div>
                  <label className="block font-bold mb-1">Cadera (cm) *</label>
                  <input type="number" value={cadera} onChange={e => setCadera(e.target.value)} className="w-full px-4 py-2 border rounded-lg" required min={40} max={200} />
                </div>
              )}
            </div>
            <div>
              <label className="block font-bold mb-1">Nivel de actividad *</label>
              <select value={actividad} onChange={e => setActividad(e.target.value)} className="w-full px-4 py-2 border rounded-lg" required>
                <option value="">Selecciona</option>
          ...
          result && (
            <div className="bg-white rounded-xl shadow p-8 max-w-2xl w-full text-center">
              <h2 className="text-2xl font-bold mb-2 text-[#8e1dd1]">Resultados Corporales</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div className="bg-[#f7fafc] rounded p-4">
              ...
              <a href="/programas" className="inline-block bg-[#8e1dd1] hover:bg-[#6c14a0] text-white px-8 py-3 rounded-lg font-bold shadow-lg transition mt-2">
