...
export default function PerfilNutricionalPage() {
  ...
  return (
    <>
      ...
      <main className="min-h-[70vh] flex flex-col items-center justify-center bg-[var(--bg)] py-16">
        <h1 className="text-3xl font-extrabold text-[var(--color-primary)] mb-4">Perfil Nutricional</h1>
  ...
  {error && <p className="text-red-500 text-center mb-4 animate-pulse">{error}</p>}
        {!enviado ? (
          ...
          resultado && (
            <div className="bg-white rounded-xl shadow p-8 max-w-xl w-full text-center border border-[var(--color-accent)]">
              <h2 className="text-2xl font-bold mb-2" style={{ color: resultado.color }}>{resultado.nivel}</h2>
              <div className="mb-2 text-gray-700">Tu IMC es: <b>{imc?.toFixed(2)}</b></div>
              <p className="text-lg text-gray-700 mb-4">{resultado.texto}</p>
              <a
                href="/programas"
                className="inline-block bg-[var(--color-primary)] hover:bg-[var(--color-accent)] text-white px-8 py-3 rounded-lg font-bold shadow-lg transition"
              >
